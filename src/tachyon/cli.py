...
@click.option("--server", default="tachyon.dev", help="Tachyon server address")
@click.option("--verbose", "-v", is_flag=True, help="Verbose output")
@click.option("--auth-token", envvar="TACHYON_AUTH_TOKEN", help="Authentication token")
...
def main(
    ...
    """Tachyon - Expose local services to the internet instantly.

    Examples:

        tachyon --port 8000

        tachyon --port 3000 --subdomain myapp

        tachyon --port 8080 --server custom.server.com

        tachyon --port 8000 --timeout 60 --idle-timeout 600
    ...
    You can run multiple tunnels simultaneously for different ports:
        ...
        Terminal 1: tachyon --port 3000 --subdomain frontend

        Terminal 2: tachyon --port 5432 --subdomain database

        Terminal 3: tachyon --port 8000 --subdomain api

    Use 'tachyon COMMAND --help' for more info on specific commands.
    """
    if ctx.invoked_subcommand is None:
        if port is None:
            console.print(BANNER, style="cyan")
            console.print("Usage: tachyon --port 8000", style="yellow")
            console.print("       tachyon --port 3000 --subdomain myapp", style="yellow")
            console.print("       tachyon --port 8000 --timeout 60", style="yellow")
            console.print("\nCommands:", style="bold")
            console.print("  tachyon status    Show server status and active tunnels", style="dim")
            console.print("  tachyon version   Show version information", style="dim")
            console.print("  tachyon http      Start HTTP tunnel (shorthand)", style="dim")
            console.print("  tachyon tcp       Start TCP tunnel", style="dim")
            return
...
async def start_tunnel(
    ...
    from tachyon.sdk import _suggest_subdomain
...
def http(port: int, subdomain: str | None, server: str, auth_token: str | None):
    ...
    Examples:
        ...
        tachyon http 3000 --subdomain myapp
    """
    asyncio.run(
        start_tunnel(
...
def tcp(port: int, remote_port: int | None, server: str, quic: bool):
    ...
    Examples:
        ...
        tachyon tcp 22                    # SSH tunnel

        tachyon tcp 5432 --remote-port 5432   # PostgreSQL

        tachyon tcp 3306                  # MySQL
    """
...
async def start_tcp_tunnel_cli(
    ...
    from tachyon.client.tcp_tunnel import TcpTunnelClient, TcpTunnelConfig
...
def udp(port: int, remote_port: int | None, server: str, quic: bool):
    ...
    Examples:
        ...
        tachyon udp 53                    # DNS tunnel

        tachyon udp 5060 --remote-port 5060   # SIP/VoIP

        tachyon udp 27015                 # Game server
    """
...
async def start_udp_tunnel_cli(
    ...
    from tachyon.client.udp_tunnel import UdpTunnelClient, UdpTunnelConfig
...
def replay(url: str, output: str | None):
    ...
    Examples:
        ...
        tachyon replay https://myapp.tachyon.dev/api/test
    """
